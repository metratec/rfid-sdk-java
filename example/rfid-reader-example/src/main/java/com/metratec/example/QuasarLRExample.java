...
import com.metratec.lib.rfidreader.iso.QuasarLRStatus;
import com.metratec.lib.tag.HfTag;

/**
 * Example class demonstrating QuasarLR Long Range Industrial HF RFID reader functionality.
 * 
 * The QuasarLR is an HF long range RFID reader/writer for demanding industrial applications
 * with high reading reliability, high read ranges and extensive special tag features.
 * Features RF power up to 4W allowing read range up to 60cm with proper antenna.
 * 
 * This class provides automated demos for:
 * - USB connection (default) with Ethernet fallback
 * - High-power long range operations (up to 4W RF power)
 * - Industrial inventory operations for ISO 15693 tags
 * - Block-based read/write operations
 * - Status monitoring and noise measurement
 * - Antenna port management
 */
public class QuasarLRExample {

    /**
     * Demonstrate QuasarLR inventory operations
     */
    public void performInventoryExample() {

        // Create the reader instance using tcp communication
        String hostname = "192.168.2.239";
        QuasarLR reader = new QuasarLR("QuasarLR", hostname, 10001);

        // Alternatively if the reader should be connected with serial
        // String serialPort = "/dev/ttyUSB0"; // Linux/Mac, use "COM#" for Windows
        // QuasarLR reader = new QuasarLR("QuasarLR", serialPort, 115200, 8, 1, 0, 0);
        ...
        try {
            System.out.println("Connecting to QuasarLR...");
            reader.startAndWait(5000);
            System.out.println("Successfully connected!");

            // Configure high power for long range operation
            System.out.println("\n--- Configuring Long Range Settings ---");
            reader.setPower(4000); // 4W for maximum range (500-8000mW in 250mW steps)
            System.out.println("Configured: Power 4000mW (4W) for long range");
            ...
            System.err.println("Please verify QuasarLR USB connection");
        } catch (RFIDReaderException e) {
            System.err.println("Reader error: " + e.getLocalizedMessage());
        } finally {
            try {
                reader.stop();
                System.out.println("Disconnected from QuasarLR reader.");
            } catch (Exception e) {
    ...
    /**
     * Demonstrate QuasarLR read/write operations
     */
    public void performReadWriteExample() {
        ...
        try {
            ...
            // Configure for optimal read/write performance
            reader.setPower(4000); // 4W for reliable read/write operations
            System.out.println("Configured: Power 2000mW (2W) for read/write operations");
    ...
    public static void main(String[] args) {
        System.out.println("=== QuasarLR Example ===");
        QuasarLRExample example = new QuasarLRExample();
