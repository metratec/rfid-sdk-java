...
/**
 * Example class demonstrating QuasarMX Industrial HF RFID reader functionality.
 * 
 * The QuasarMX is an HF RFID reader/writer for demanding industrial applications with high reading reliability, speed
 * and extensive special tag features. Features include reading rate up to 100 tag-IDs/sec and bulk read/write
 * operations without individual tag addressing.
 * 
 * This class provides automated demos for: - USB connection (default) with Ethernet and Serial fallback - High-speed
 * industrial operations (up to 100 tag-IDs/sec) - Bulk inventory operations for ISO 15693 tags - Block-based read/write
 * operations - Conveyor belt and production machinery integration - Power control for different hardware revisions
 */
public class QuasarMXExample {

    /**
     * Demonstrate QuasarMX inventory operations
     */
    public void performInventoryExample() {

        // Create the reader instance using tcp communication
        String hostname = "192.168.2.239";
        QuasarMX reader = new QuasarMX("QuasarMX", hostname, 10001);

        // Alternatively if the reader should be connected with serial
        // String serialPort = "/dev/ttyUSB0"; // Linux/Mac, use "COM#" for Windows
        // QuasarMX reader = new QuasarMX("QuasarMX", serialPort, 115200, 8, 1, 0, 0);
        ...
        try {
            System.out.println("Connecting to QuasarMX...");
            ...
            try {
                reader.stop();
                System.out.println("Disconnected from QuasarMX reader.");
            } catch (Exception e) {
    ...
    /**
     * Demonstrate QuasarMX read/write operations
     */
    ...
    public static void main(String[] args) {
        System.out.println("=== QuasarMX Example ===");
        QuasarMXExample example = new QuasarMXExample();
