...
import com.metratec.lib.rfidreader.MetratecReaderAscii;
 ...
 * 
 */
public class ISOReaderAscii extends MetratecReaderAscii<HfTag> {
  private final Logger logger = LoggerFactory.getLogger(ISOReaderAscii.class);
  private MODE mode;
  private SRI sri;
  private int afi = 0;
  ...
  private Object handleInventoryLock = new Object();
   ...
   */
  protected ISOReaderAscii(String identifier, ICommConnection connection, String... minReaderRevision) {
   ...
   * @param minReaderRevision minimal reader revision
   */
  protected ISOReaderAscii(String identifier, ICommConnection connection, MODE mode, SRI sri,
   ...
   */
  @Deprecated
  public ISOReaderAscii(String identifier, String usbDeviceSerialNumber, MODE mode, SRI sri) {
   ...
   */
  @Deprecated
  public ISOReaderAscii(String identifier, String ipAddress, int port, MODE mode, SRI sri) {
   ...
   */
  @Deprecated
  public ISOReaderAscii(String identifier, String portName, int baudrate, int dataBit, int stopBit, int parity,
  ...
  /**
   * Use {@link ISOReaderAscii#stop()} for close the connection
   * 
  ...
  private List<HfTag> getInventory(boolean ssl, boolean ont) throws CommConnectionException, RFIDReaderException {
    ...
    try {
      ...
      synchronized (handleInventoryLock) {
        try {
          handleInventoryLock.wait(getReceiveTimeout());
  ...
  /**
   * Send a writing request directly to tags (DWQ). Use this if the data on the tag is to be changed/written. With this
   * method you can send manufacturer specific commands.
   * 
   * @param tagCommand the command defined in the ISO 15693-3. For example '022100A000B000' to write 'A000B000' to the
   *        block 0 of a tag
   * @throws RFIDReaderException possible RFIDErrorCodes:
  ...
  public HFTagInformation getTagSystemInformation(String tagID) throws RFIDReaderException, CommConnectionException {
    ...
    byte infoFlag;
    try {
      infoFlag = (byte) Integer.parseInt(answer.substring(0, 2), 16);
    } catch (NumberFormatException e) {
      throw new RFIDReaderException(RFIDErrorCodes.NER,
          "Invalid info flag format in system information: " + e.getMessage());
    }
    ...
    if (1 == (infoFlag & 0x01)) {
      ...
      try {
        ...
        throw new RFIDReaderException(RFIDErrorCodes.NER,
            "Invalid DSFID format in system information: " + e.getMessage());
      }
    }
    if (2 == (infoFlag & 0x02)) {
      isAFI = true;
      try {
        ...
        throw new RFIDReaderException(RFIDErrorCodes.NER,
            "Invalid AFI format in system information: " + e.getMessage());
      }
    }
    if (4 == (infoFlag & 0x04)) {
      isVICCMS = true;
      try {
        ...
        throw new RFIDReaderException(RFIDErrorCodes.NER,
            "Invalid VICC memory size format in system information: " + e.getMessage());
      }
    }
    if (8 == (infoFlag & 0x08)) {
      isICR = true;
      try {
        ...
        throw new RFIDReaderException(RFIDErrorCodes.NER,
            "Invalid ICR format in system information: " + e.getMessage());
      }
    }
  ...
  /**
   * The reader allows different output power levels to match antenna size, tag size or tag position. The power level is
   * given in milliwatt (mW). The minimum value is 500, the maximum is 4000 with steps of 250.
   * 
   ...
   */
  public void startInventory(long tagLostTime, boolean ssl, boolean ont)
      throws CommConnectionException, RFIDReaderException {
    if (scanningForTags) {
