...
/**
 * Abstract base class for Metratec RFID readers that use the AT command protocol.
 * This class extends MetratecReader and provides specific functionality for readers
 * that communicate using AT-style commands. It handles command formatting, response
 * parsing, antenna management, and echo control for AT protocol-based communication.
 * 
 * @param <T> the type of RFID tag handled by this reader
 * @author Matthias Neumann (neumann@metratec.com)
 */
public abstract class MetratecReaderAT<T extends RfidTag> extends MetratecReader<T> {
   ...
   */
  public MetratecReaderAT(String identifier, ICommConnection connection) {
          ...
          switch (resp.charAt(0)) {
            ...
            case 'A': // Echo
              if (!command.startsWith(resp)) {
                throw new RFIDReaderException(RFIDErrorCodes.NER, "Wrong response to " + command + " - " + resp);
              }
              break;
          }
          data += resp;
  ...
  /**
   * @param data reader error message
   * @return the parse reader exception
   */
  protected abstract RFIDReaderException parseErrorResponse(String data);
  ...
  public List<T> getInventory() throws RFIDReaderException, CommConnectionException {
    if (useSingleAntenna) {
      ...
      return getMultiplexInventory();
    }
  }

  /**
   * Looks for all tags in range of the reader and get all tags back.<br>
   * Using the last configured antenna port.<br>
   ...
   * @throws RFIDReaderException if an protocol exception occurs (e.g. CRC error, value out of range, ..)
   */
  public List<T> getSingleInventory() throws RFIDReaderException, CommConnectionException {
    if (getInternalInventory().isAlive()) {
      return getInternalInventory().getInventory();
    } else {
      List<T> inv = parseInventory(splitResponse(communicateSynchronized("AT+INV")), 6, true); // prefix_length
   ...
   * @throws RFIDReaderException if an protocol exception occurs (e.g. CRC error, value out of range, ..)
   */
  public List<T> getMultiplexInventory() throws RFIDReaderException, CommConnectionException {
    return getMultiplexInventory(true);
  }
   ...
   * @throws RFIDReaderException if an protocol exception occurs (e.g. CRC error, value out of range, ..)
   */
  public List<T> getMultiplexInventory(boolean throwAntennaErrors) throws RFIDReaderException, CommConnectionException {
    ...
    if (getInternalInventory().isAlive()) {
      ...
      addNewInventoryEvent(inventory);
      return inventory;
    }
  }
  ...
  public List<T> getInventoryMultiplex(boolean throwAntennaErrors) throws RFIDReaderException, CommConnectionException {
    return getMultiplexInventory(throwAntennaErrors);
  }
   ...
   * Reset the reader
   * 
   * @param delay reboot time of the device
   * 
   * @throws RFIDReaderException if a error occurs, see {@link RFIDReaderException#getErrorCode()} and
   *         {@link RFIDReaderException#getMessage()} for more details
   * @throws CommConnectionException if a error occurs, see {@link CommConnectionException#getErrorCode()} and
   *         {@link CommConnectionException#getMessage()} for more details
   */
  public void reset(long delay) throws RFIDReaderException, CommConnectionException {
    communicateSynchronized("AT+RST");
    closeConnection();
    try {
      TimeUnit.MILLISECONDS.sleep(delay);
    } catch (InterruptedException e) {
      getLogger().trace("sleep interrupted {}", e.getMessage());
    }
    startAndWait();
    prepareDevice();
  }
    ...
    switch (response.charAt(0)) {
      ...
      case '+':
        ...
        try {
          switch (response.charAt(1)) {
            ...
            case 'C':
              ...
              try {
                ...
                } else if (response.length() > 5 && response.charAt(5) == 'R') {
          ...
          getLogger().debug("Problem during parsing event - " + response + " - " + e.getMessage());
        }
        break;
  ...
  @Override
  public void setHeartbeatInterval(int intervalInSeconds) throws RFIDReaderException, CommConnectionException {
  ...
  public int getAntennaPort() throws CommConnectionException, RFIDReaderException {
    ...
    try {
      ...
      throw new RFIDReaderException(RFIDErrorCodes.NER, "Invalid antenna port response format: " + e.getMessage());
    }
  }
  ...
  public void startInventory(long tagLostTime) throws CommConnectionException, RFIDReaderException {
    if (useSingleAntenna) {
      startSingleInventory(tagLostTime);
    } else {
      startMultiplexInventory(tagLostTime);
    }
  }
  ...
  /**
   * Looks for all tags in range of the reader and call events with founded tags. Using the last configured antenna
   * port.
   * 
   * @throws CommConnectionException if an communication exception occurs
   * @throws RFIDReaderException if an protocol exception occurs (e.g. CRC error, value out of range, ..)
   */
  public void startSingleInventory() throws CommConnectionException, RFIDReaderException {
    startSingleInventory(DEFAULT_TAG_LOST_TIME);
  }
   ...
   * @throws RFIDReaderException if an protocol exception occurs (e.g. CRC error, value out of range, ..)
   */
  public void startSingleInventory(long tagLostTime) throws CommConnectionException, RFIDReaderException {
    ...
    communicateSynchronized("AT+CINV");
  }
   ...
   */
  public List<T> stopSingleInventory() throws CommConnectionException, RFIDReaderException {
    return stopInventory();
  }

  /**
   * Looks for all tags in range of the reader and call events with founded tags. Using the last configured multiplex
   * antennas.
   * 
   * @throws CommConnectionException if an communication exception occurs
   * @throws RFIDReaderException if an protocol exception occurs (e.g. CRC error, value out of range, ..)
   */
  public void startMultiplexInventory() throws CommConnectionException, RFIDReaderException {
    startMultiplexInventory(DEFAULT_TAG_LOST_TIME);
  }
   ...
   * @throws RFIDReaderException if an protocol exception occurs (e.g. CRC error, value out of range, ..)
   */
  public void startMultiplexInventory(long tagLostTime) throws CommConnectionException, RFIDReaderException {
    if (null == getInternalInventory()) {
      throw new RFIDReaderException(RFIDErrorCodes.TLM, "tag listener missing");
    }
    getInternalInventory().setKeepTime(tagLostTime);
    getInternalInventory().start();
    communicateSynchronized("AT+CMINV");
  }
   ...
   * @throws RFIDReaderException if an protocol exception occurs
   */
  public List<T> stopMultiplexInventory() throws CommConnectionException, RFIDReaderException {
    try {
      communicateSynchronized("AT+BMINV");
    } catch (RFIDReaderException e) {
      if (!e.getMessage().contains("is not running")) {
        throw e;
      }
